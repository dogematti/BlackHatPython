...
def parallel_https_get_request(target_url, num_requests, delay, num_threads, verify_ssl=True, verbose=False):
    ...
    def single_request():
        try:
            response = requests.get(target_url, headers=headers, verify=verify_ssl, timeout=10)
...
def parse_args():
    ...
    parser.add_argument("-p", "--min_packet_size", type=int, default=64, nargs='?', help="Minimum packet size for UDP (bytes)")
    parser.add_argument("-q", "--max_packet_size", type=int, default=1024, nargs='?', help="Maximum packet size for UDP (bytes)")
    ...
    elif not validate_ip(args.target_host):
        raise ValueError("Invalid IP address format.")

    if args.test_type == "http" and args.http_url and not validate_url(args.http_url):
